...
namespace kagome {
  ...
  using storage::trie::PolkadotTrieFactoryImpl;
  using storage::trie::RootHash;
  using storage::trie::TrieBatch;
  using storage::trie::TrieSerializer;
  using storage::trie::TrieSerializerImpl;
...
Example:
  ...
  get_persistent_batch_and_track_used_nodes(TrieStorage &trie,
                                            const RootHash &hash) {
  ...
  void fill_with_child_storage_root_hashes(TrieBatch &batch,
  ...
  int db_editor_main(int argc, const char **argv) {
    ...
    if (argc == 2 or (argc == 3 && is_hash(args[2]))
        or (argc == 4 and std::strcmp(args[MODE], "compact") == 0)) {
      cmd = COMPACT;
    } else if (argc == 4 and std::strcmp(args[MODE], "dump") == 0) {
      cmd = DUMP;
    } else {
      usage();
      return 0;
    ...
    if (argc > 2) {
      ...
      target_state_param = RootHash::fromHexWithPrefix(args[2]).value();
    }
    ...
    bool need_additional_compaction = false;
    {
      ...
      SL_INFO(log, "Save {} as single leaf", last_finalized_block);
      ...
      if (COMPACT == cmd) {
        auto batch = check(get_persistent_batch_and_track_used_nodes(
                               *trie, target_state))
                         .value();
        ...
        std::vector<std::unique_ptr<TrieBatch>> child_batches;
        {
          ...
          fill_with_child_storage_root_hashes(*batch, child_root_hashes);
          for (const auto &child_root_hash : child_root_hashes) {
            auto child_batch_res = get_persistent_batch_and_track_used_nodes(
                *trie, child_root_hash);
        ...
        auto track_trie_entries = [&log, &buffer_storage, &prefix](
                                      auto storage, auto tracker) {
        ...
        auto res = check(cursor->next());
        {
          ...
          while (cursor->key().has_value()) {
            ofs << "  - "
                << check(batch->get(check(cursor->key()).value()))
                       .value()
                       .view()
                << "\n";
            if (++count % 10000 == 0) {
    ...
    if (need_additional_compaction) {
      ...
      auto storage =
          check(storage::RocksDb::create(args[1], rocksdb::Options())).value();
