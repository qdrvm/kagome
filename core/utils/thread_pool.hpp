...
#define REINVOKE_6(ctx, func, in_1, in_2, in_3, in_4, in_5, in_6, out_1, out_2, out_3, out_4, out_5, out_6) \
  auto res##func = (ctx).reinvoke(                                   \
      [wself(weak_from_this())](auto &&x1, auto &&x2, auto &&x3, auto &&x4, auto &&x5, auto &&x6) {   \
        if (auto self = wself.lock()) {                              \
          self->func(std::move(x1), std::move(x2), std::move(x3), std::move(x4), std::move(x5), std::move(x6));   \
        }                                                            \
      },                                                             \
      std::move(in_1),                                               \
      std::move(in_2),                                               \
      ...
      std::move(in_6));                                              \
  if (!(res##func)) return;                                          \
  auto &&[func##x1, func##x2, func##x3, func##x4, func##x5, func##x6] = std::move(*(res##func));   \
  auto && (out_1) = func##x1;                                        \
  auto && (out_2) = func##x2;                                        \
  auto && (out_3) = func##x3; \
  auto && (out_4) = func##x4; \
...
#define REINVOKE_4(ctx, func, in_1, in_2, in_3, in_4, out_1, out_2, out_3, out_4) \
  auto res##func = (ctx).reinvoke(                                   \
      [wself(weak_from_this())](auto &&x1, auto &&x2, auto &&x3, auto &&x4) {   \
        if (auto self = wself.lock()) {                              \
          self->func(std::move(x1), std::move(x2), std::move(x3), std::move(x4));   \
        }                                                            \
      },                                                             \
      std::move(in_1),                                               \
      std::move(in_2),                                               \
