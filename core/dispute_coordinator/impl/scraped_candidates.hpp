...
namespace kagome::dispute {
  ...
  class ScrapedCandidates final {
    ...
    //    bool is_candidate_included(const CandidateHash &candidate_hash)
    //    override;
    //    /// Check whether the candidate is backed
    //    bool is_candidate_backed(const CandidateHash &candidate_hash)
    //    override;
    //
    //   private:
    //    /// Main data structure which keeps the candidates we know about.
    //    `contains`
    //    /// does lookups only here.
    //    RefCountedCandidates candidates;
    //    /// Keeps track at which block number a candidate was inserted. Used
    //    in
    //    /// `remove_up_to_height`. Without this tracking we won't be able to
    //    remove
    //    /// all candidates before block X.
    //    BTreeMap<BlockNumber, HashSet<CandidateHash>>
