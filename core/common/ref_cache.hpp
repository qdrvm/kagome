...
namespace kagome {
    ...
    /**
     * @brief Cache keeps an item inside if any reference exists. If all references lost, then item removes from cache. If an item exists, then it returns new reference on it.
    */
   template<typename K, typename V>
    struct RefCache : std::enable_shared_from_this<RefCache> {
        ...
        */
        ...
        /**
         * @brief returns an existed element or creates new one and store it in cache.
         * @param k is a key of an element
        ...
        */
        ...
        SRefObj get_or_insert(const Key &k, F &&f) {
            ...
            auto obj = std::make_shared(std::forward<F>(f)());
            auto [it, inserted] = items_.emplace(k, obj);
            BOOST_ASSERT(inserted);

            obj->store_internal_refs(it, shared_from_this());
            return obj;
        }
