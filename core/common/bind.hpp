...
#ifndef KAGOME_BIND_HPP
#define KAGOME_BIND_HPP
...
namespace kagome {
   ...
   *
   * std::optional<int> f();
   * std::optional<double> g(int);
   *
   * std::optional<double> d = f()
   *    | g;
   *
   ...
   */
  template <typename T, typename Transform>
  auto operator|(T &&t, Transform &&f) -> std::enable_if_t<std::is_same<
      decltype(std::forward<Transform>(f)(*std::forward<T>(t))), void>::value> {
    if (std::forward<T>(t)) {
