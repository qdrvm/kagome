...
namespace kagome::injector {
  ...
  // peer info getter
  template <typename Injector>
  sptr<libp2p::peer::PeerInfo> get_peer_info(const Injector &injector) {
    static auto initialized =
  ...
  template <typename Injector>
  sptr<primitives::AuthorityIndex> get_authority_index(
  ...
  template <typename Injector>
  sptr<api::Listener> get_extrinsic_api_listener(const Injector &injector) {
    static auto initialized = boost::optional<sptr<api::Listener>>(boost::none);
  ...
  template <typename Injector>
  sptr<storage::trie::PolkadotTrieDbBackend> get_polkadot_trie_db_backend(
      const Injector &injector) {
    static auto initialized =
  ...
  template <typename Injector>
  sptr<storage::trie::PolkadotTrieDb> get_polkadot_trie_db(
      const Injector &injector) {
    static auto initialized =
    ...
    auto backend =
        injector.template create<sptr<storage::trie::PolkadotTrieDbBackend>>();
  ...
  // trie db getter
  template <typename Injector>
  sptr<storage::trie::TrieDb> get_trie_db(const Injector &injector) {
    static auto initialized =
  ...
  // level db getter
  template <typename Injector>
  sptr<storage::PersistentBufferMap> get_level_db(std::string_view leveldb_path,
                                                  const Injector &injector) {
    static auto initialized =
  ...
  auto makeApplicationInjector(const std::string &genesis_path,
    ...
    return di::make_injector(
        ...
        di::bind<crypto::SR25519Keypair>.to(
            [](const auto &injector) { return get_sr25519_keypair(injector); }),
        // bind ed25519 keypair
        di::bind<crypto::ED25519Keypair>.to(
            [](const auto &injector) { return get_ed25519_keypair(injector); }),
        // compose peer info
        di::bind<libp2p::peer::PeerInfo>.to(
            [](const auto &injector) { return get_peer_info(injector); }),
        // bind boot nodes
        di::bind<network::PeerList>.to(
            [](const auto &injector) { return get_boot_nodes(injector); }),
        // find and bind authority id
        di::bind<primitives::AuthorityIndex>.to(
            [](const auto &injector) { return get_authority_index(injector); }),
        ...
        // bind interfaces
        di::bind<api::Listener>.to(
            [](const auto &injector) { return get_extrinsic_api_listener(injector); }),
        ...
        di::bind<storage::PersistentBufferMap>.to(
            [leveldb_path](const auto &injector) { return get_level_db(leveldb_path, injector); }),
        ...
        di::bind<storage::trie::PolkadotTrieDbBackend>.to(
            [](const auto &injector) {
              return get_polkadot_trie_db_backend(injector);
            }),
        di::bind<storage::trie::PolkadotTrieDb>.to(
            [](const auto &injector) { return get_polkadot_trie_db(injector); }),
        di::bind<storage::trie::TrieDb>.to(
            [](const auto &injector) { return get_trie_db(injector); }),
        ...
        di::bind<application::ConfigurationStorage>.to(
            [genesis_path](const auto &injector) { return get_configuration_storage(genesis_path, injector); }),
        di::bind<application::KeyStorage>.to(
            [keystore_path](const auto &injector) { return get_key_storage(keystore_path, injector); }),
