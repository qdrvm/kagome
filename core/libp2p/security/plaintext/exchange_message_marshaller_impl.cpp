...
OUTCOME_CPP_DEFINE_CATEGORY(libp2p::security::plaintext,
  ...
  switch (e) {
    ...
    case E::MESSAGE_SERIALIZING_ERROR:
      return "Error while encoding the plaintext exchange message to Protobuf format";
  }
...
namespace libp2p::security::plaintext {
  ...
  outcome::result<ExchangeMessage> ExchangeMessageMarshallerImpl::unmarshal(
    ...
    std::vector<uint8_t> pubkey_bytes(exchange_msg.ByteSizeLong());
    exchange_msg.SerializeToArray(pubkey_bytes.data(), pubkey_bytes.size());
    ...
    return ExchangeMessage{.pubkey = pubkey,
                           .peer_id = peer_id};
  }
