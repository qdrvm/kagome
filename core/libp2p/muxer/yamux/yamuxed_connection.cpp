...
OUTCOME_CPP_DEFINE_CATEGORY(libp2p::muxer, YamuxedConnection::YamuxErrorStream,
                            e) {
  using ErrorType = libp2p::muxer::YamuxedConnection::YamuxErrorStream;
  switch (e) {
    ...
    case ErrorType::YAMUX_IS_CLOSED:
      return "this YamuxedConnection instance is closed";
  }
...
namespace libp2p::muxer {
  YamuxedConnection::YamuxedConnection(
      ...
      MuxerAdaptor::NewStreamHandler stream_handler,
  ...
  YamuxedConnection::~YamuxedConnection() = default;

  void YamuxedConnection::start() {
    startReadingHeader();
  }

  void YamuxedConnection::startReadingHeader() {
    if (!is_active_) {
      return;
    }
  ...
  void YamuxedConnection::readingHeaderCompleted(const std::error_code &ec,
                                                 size_t n) {
    if (ec) {
  ...
  void YamuxedConnection::readingDataCompleted(const std::error_code &ec,
                                               size_t n,
  ...
  void YamuxedConnection::write(const kagome::common::Buffer &msg,
  ...
  void YamuxedConnection::startWriting() {
    if (!is_active_) {
      return;
    }
    if (connection_->isClosed()) {
  ...
  void YamuxedConnection::writingCompleted(
  ...
  bool YamuxedConnection::isClosed() {
    return connection_->isClosed();
  }

  void YamuxedConnection::closeYamux() {
    streams_.clear();
  ...
  bool YamuxedConnection::processData(YamuxStreamParameters &stream,
  ...
  YamuxedConnection::processAck(StreamId stream_id) {
  ...
  bool YamuxedConnection::processHeader() {
  ...
  bool YamuxedConnection::processDataFrame(const YamuxFrame &frame) {
  ...
  void YamuxedConnection::streamReadFrameAsync(
      StreamId stream_id,
  ...
  void YamuxedConnection::streamWriteFrameAsync(
  ...
  void YamuxedConnection::streamClose(StreamId stream_id) {
  ...
  bool YamuxedConnection::streamIsClosedForWrite(StreamId stream_id) {
    auto stream = findStream(stream_id);
    if (stream) {
  ...
  bool YamuxedConnection::streamIsClosedForRead(StreamId stream_id) {
    auto stream = findStream(stream_id);
    if (stream) {
  ...
  bool YamuxedConnection::streamIsClosedEntirely(StreamId stream_id) {
