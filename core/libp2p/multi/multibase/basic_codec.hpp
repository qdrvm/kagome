...
namespace multibase {
  ...
  class basic_codec : public codec::impl {
   ...
   private:
    ...
    constexpr static iterator find(iterator b,
                                   iterator e,
  ...
  std::size_t basic_codec<T, Traits>::encode(const cstring_span &input,
    ...
    for (auto end = std::end(input); ii != end; ++ii) {
      ...
      for (auto oi = output.rbegin();
           (oi != output.rend()) && (carry != 0 || i < length);
           ++oi, ++i) {
        carry += 256 * (*oi);
    ...
    std::transform(it, output.end(), &output[zeroes], [](auto c) {
      return Traits::charset[c];
    });
  ...
  std::size_t basic_codec<T, Traits>::decode(const cstring_span &input,
    ...
    for (auto end = input.end(); ii != end && *ii; ++ii) {
      ...
      if (carry == 255)
        throw std::range_error("Invalid input character");
      int i = 0;
      for (auto oi = output.rbegin();
           (carry != 0 || i < length) && (oi != output.rend());
           ++oi, ++i) {
        auto byte = (unsigned char *)(&(*oi));
        carry += radix * (*byte);
    ...
    auto it = std::find_if(
        output.begin() + (output.size() - length), output.end(), not_zero);
  ...
  bool basic_codec<T, Traits>::is_valid(const cstring_span &input, impl_tag) {
    return input.end() == std::find_if(input.begin(), input.end(), [](auto c) {
             return valset[c] == 255;
           });
  }
  ...
  struct traits<encoding::base_16> {
    constexpr static const std::array<char, 16> charset = {'0',
                                                           '1',
                                                           '2',
                                                           '3',
                                                           '4',
