...
namespace kagome::parachain {
  BitfieldStoreImpl::BitfieldStoreImpl(std::shared_ptr<runtime::ParachainHost> parachain_api)
  ...
  void BitfieldStoreImpl::putBitfield(const BlockHash &relay_parent,
    ...
    logger_->trace("Put bitfields.(relay_parent={}, bitfields={})", relay_parent, bitfield);
  ...
  std::vector<BitfieldStore::SignedBitfield> BitfieldStoreImpl::getBitfields(
    ...
    if (c.has_error()) {
      logger_->warn("Availability cores not present.(relay parent={}, error={})",
                    relay_parent, c.error().message());
      return {};
    }
    auto &cores = c.value();
    ...
    for (auto const &bf: it->second) {
      if (bf.payload.payload.bits.size() != cores.size()) {
        logger_->warn("dropping bitfield due to length mismatch.(relay parent={})",
                      relay_parent);
        continue;
      }

      auto it_selected = selected.find(bf.payload.ix);
      if (it_selected == selected.end() || approval::count_ones(it_selected->second.payload.payload) < approval::count_ones(bf.payload.payload)) {
        bool skip = false;
        for (size_t ix = 0; ix < cores.size(); ++ix) {
          ...
          if (bf.payload.payload.bits[ix]) {
            logger_->debug("dropping invalid bitfield - bit is set for an unoccupied core.(relay_parent={})", relay_parent);
            skip = true;
            break;
          }
        }
