...
namespace {
  ...
  kagome::network::Tick slot_number_to_tick(
      uint64_t slot_duration_millis, kagome::consensus::BabeSlotNumber slot) {
...
namespace kagome::parachain {
  ...
  bool ApprovalDistribution::prepare() {
    BOOST_ASSERT(nullptr == int_pool_);
    int_pool_ = std::make_shared<thread::ThreadPool>(1ull);
    if (!int_pool_->prepare()) {
      return false;
    }
  ...
  bool ApprovalDistribution::start() {
    if (int_pool_) return int_pool_->start();
    return false;
  }

  void ApprovalDistribution::stop() {
    if (int_pool_) int_pool_->stop();
    int_pool_.reset();
  }
  ...
  void ApprovalDistribution::store_included_candidates(
      ...
      ApprovalDistribution::CandidateIncludedList &&candidates_list) {
    ApprovingContext &context = acu.second;
    context.included_candidates = std::move(candidates_list);
  }
  ...
  void ApprovalDistribution::try_process_approving_context(
    ...
    switch (ac.babe_block_header->slotType()) {
      case consensus::SlotType::Primary:
      case consensus::SlotType::SecondaryVRF:
        break;

      case consensus::SlotType::SecondaryPlain:
      default:
        return;
    }
    ...
    thread_pool_->template execute(std::make_pair(
        thread::createThreadQueueContext(ac.complete_callback_context),
        [callback{std::move(ac.complete_callback)},
         imported_block{std::move(imported_block)}]() mutable {
  ...
  void ApprovalDistribution::request_session_index_and_info(
    ...
    thread_pool_->template execute(
        ...
        std::make_pair(
            createThreadQueueContext(*int_pool_),
            [block_hash, wptr{weak_from_this()}](
                std::pair<SessionIndex, runtime::SessionInfo> &&session_data) {
              if (auto self = wptr.lock()) {
  ...
  void ApprovalDistribution::request_babe_epoch_and_block_header(
    ...
    thread_pool_->template execute(
        std::make_pair(
            kagome::thread::createThreadQueueContext(exec_context),
            [babe_util{babe_util_}, block_header]()
                -> outcome::result<std::pair<consensus::EpochNumber,
                                             consensus::BabeBlockHeader>> {
              OUTCOME_TRY(babe_digests,
                          consensus::getBabeDigests(block_header));
              const consensus::EpochNumber epoch_number =
        ...
        std::make_pair(
            createThreadQueueContext(*int_pool_),
            [block_hash, wptr{weak_from_this()}](
                std::pair<consensus::EpochNumber, consensus::BabeBlockHeader>
                    &&babe_block_data) {
              if (auto self = wptr.lock()) {
                self->template for_ACU(block_hash, [&](auto &acu) {
  ...
  void ApprovalDistribution::request_included_candidates(
      const primitives::BlockHash &block_hash) {
    thread_pool_->template execute(
        [hasher{hasher_}, block_hash, parachain_host{parachain_host_}]()
            -> outcome::result<ApprovalDistribution::CandidateIncludedList> {
          OUTCOME_TRY(candidates, parachain_host->candidate_events(block_hash));
        ...
        std::make_pair(createThreadQueueContext(*int_pool_),
                       [block_hash, wptr{weak_from_this()}](
                           ApprovalDistribution::CandidateIncludedList
                               &&candidates_included_list) {
                         if (auto self = wptr.lock())
                           self->template for_ACU(block_hash, [&](auto &acu) {
  ...
  void ApprovalDistribution::handle_new_head(const primitives::BlockHash &head,
                                             ...
                                             Func &&func) {
    int_pool_->template execute(
        std::make_pair(thread::createThreadQueueContext(this_context_),
                       [block_tree{block_tree_}, head]() {
                         return block_tree->getBlockHeader(head);
                       }),
        [head, wself{weak_from_this()}, func(std::forward<Func>(func))](
            primitives::BlockHeader &&header) mutable -> outcome::result<void> {
              ...
              [wself,
               ...
               func(std::forward<Func>(func))](
                ...
                std::forward<Func>(func)(self->processImportedBlock(
                    block_number, block_hash, std::move(block_info.value())));
              });
  ...
  void ApprovalDistribution::on_active_leaves_update(
    ...
    for (auto const &head : heads) {
      handle_new_head(
          head,
          finalized,
          [wself{weak_from_this()}, head](BlockImportedCandidates &&candidate) {
