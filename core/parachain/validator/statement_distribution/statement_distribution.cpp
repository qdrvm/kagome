  ...
  StatementDistribution::handle_active_leaves_update_inner(
    ...
    for (const auto &new_context : new_contexts) {
      ...
      if (maybe_claim_queue) {
        // Get scheduling_lookahead from runtime API if possible, otherwise use default
        auto scheduling_lookahead_result = parachain_host->scheduling_lookahead(relay_parent);
        uint32_t scheduling_lookahead = scheduling_lookahead_result.has_value() 
            ? scheduling_lookahead_result.value() 
            : parachain::DEFAULT_SCHEDULING_LOOKAHEAD;
            
        transposed_claim_queue = transposeClaimQueue(*maybe_claim_queue, scheduling_lookahead);
      }
