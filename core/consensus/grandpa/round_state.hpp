...
//                         round_state.finalized);
  ...
  Stream &operator<<(Stream &s, const RoundState &state) {
    return s
//    << state.last_finalized_block << state.best_prevote_candidate << state.best_final_candidate << state.finalized
             ;
  }
  ...
  Stream &operator>>(Stream &s, RoundState &state) {
    return s
//    >> state.last_finalized_block >> state.best_prevote_candidate
