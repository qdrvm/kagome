...
void registerHostApiFunc(const std::string &name,
  ...
  WasmEdge_FunctionInstanceContext *HostFunc = WasmEdge_FunctionInstanceCreate(
      HostFType, HostApiFunc<decltype(mf), mf>::fun(), host_api, 0);
...
#define REGISTER_HOST_API_FUNC(name) \
  registerHostApiFunc<&HostApi ::name>(#name, ImpObj, host_api);
...
inline void register_host_api(WasmEdge_ImportObjectContext *ImpObj,
  ...
  REGISTER_HOST_API_FUNC(ext_offchain_local_storage_compare_and_set_version_1);
