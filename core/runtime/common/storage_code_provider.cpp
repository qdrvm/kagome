...
namespace kagome::runtime {
  ...
  RuntimeCodeProvider::Result StorageCodeProvider::getCodeAt(
    ...
    if (last_state_root_ != state) {
      ...
      const auto state_api = state_api_.get();
      if (state_api) {
        const auto runtime_version_res = state_api->getRuntimeVersion(state);
        if (runtime_version_res.has_value()) {
          system_version = runtime_version_res.value().state_version;
        }
      }
