 ...
 * filename
 */
TEST_F(CryptoStoreTest, succeeds_empty_for_0_cores) {
...
TEST_F(CryptoStoreTest, assign_to_nonzero_core) {
  auto cs = create_crypto_store();
  auto asgn_keys =
      assignment_keys_plus_random(cs, {"//Alice", "//Bob", "//Charlie"}, 0ull);
...
TEST_F(CryptoStoreTest, assignments_produced_for_non_backing) {
  auto cs = create_crypto_store();
  auto asgn_keys =
      assignment_keys_plus_random(cs, {"//Alice", "//Bob", "//Charlie"}, 0ull);
