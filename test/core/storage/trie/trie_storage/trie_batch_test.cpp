...
TEST_F(TrieBatchTest, Put) {
  auto batch = trie->getPersistentBatchAt(empty_hash, kNoFn).value();
  FillSmallTrieWithBatch(*batch);
  // changes are not yet commited
  auto new_batch = trie->getEphemeralBatchAt(empty_hash, kNoFn).value();
  for (auto &entry : data) {
  ...
  new_batch = trie->getEphemeralBatchAt(root_hash, kNoFn).value();
  for (auto &entry : data) {
...
TEST_F(TrieBatchTest, Remove) {
  ...
  auto read_batch = trie->getEphemeralBatchAt(root_hash, kNoFn).value();
  for (auto i : {2, 3, 4}) {
...
TEST_F(TrieBatchTest, TopperBatchAtomic) {
  std::shared_ptr<PersistentTrieBatch> p_batch =
      trie->getPersistentBatchAt(empty_hash, kNoFn).value();
