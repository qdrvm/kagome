...
static const std::vector<Case> CASES = {
    {make<LeafNode>({}, std::nullopt), {LEAF}},                        // 0
    {make<LeafNode>({0}, std::nullopt), {LEAF | 1u}},                  // 1
    {make<LeafNode>({0, 0, 0xf, 0x3}, std::nullopt), {LEAF | 4u}},     // 2
    {make<LeafNode>(Buffer(62, 0xf), std::nullopt), {LEAF | 62u}},     // 3
    {make<LeafNode>(Buffer(63, 0xf), std::nullopt), {LEAF | 63u, 0}},  // 4
    ...
    {make<BranchNode>({}, std::nullopt), {BRANCH_NO_VAL}},        // 8
    {make<BranchNode>({0}, std::nullopt), {BRANCH_NO_VAL | 1u}},  // 9
    {make<BranchNode>({0, 0, 0xf, 0x3}, std::nullopt),
     {BRANCH_NO_VAL | 4u}},                                            // 10
    ...
    {make<BranchNode>(Buffer(62, 0xf), std::nullopt),
     {BRANCH_NO_VAL | 62u}},  // 15
    {make<BranchNode>(Buffer(62, 0xf), Buffer{0x01}),
     {BRANCH_VAL | 62u}},  // 16
    {make<BranchNode>(Buffer(63, 0xf), std::nullopt),
     {BRANCH_NO_VAL | 63u, 0}},  // 17
    {make<BranchNode>(Buffer(64, 0xf), std::nullopt),
