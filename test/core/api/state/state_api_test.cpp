...
class TrieMockBuilder: public ReadonlyTrieBuilder {
 public:
  ...
  std::unique_ptr<TrieDbReader> buildAt(
      BlockHash state_root) const override {
...
TEST(StateApiTest, GetStorage) {
  ...
  EXPECT_CALL(*block_tree, getLastFinalized()).WillOnce(testing::Return("D"_hash256));
  ...
  EXPECT_CALL(*builder->next_trie, get("a"_buf)).WillOnce(testing::Return("1"_buf));
