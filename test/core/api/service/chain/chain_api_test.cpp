 ...
 */
TEST(ChainApiTest, GetBlockHashByNumber) {
  auto block_header_repo = std::make_shared<HeaderRepositoryMock>();
  auto block_tree = std::make_shared<BlockTreeMock>();
  ...
  EXPECT_CALL(*block_header_repo, getHashByNumber(42))
 ...
 * @then the correct hash value is returned
 */
TEST(ChainApiTest, GetBlockHashByHexNumber) {
  auto block_header_repo = std::make_shared<HeaderRepositoryMock>();
  auto block_tree = std::make_shared<BlockTreeMock>();
 ...
 */
TEST(ChainApiTest, GetBlockHashArray) {
  auto header_repo = std::make_shared<HeaderRepositoryMock>();
  auto block_tree = std::make_shared<BlockTreeMock>();

  ChainApiImpl api{header_repo, block_tree};
  ...
  std::vector<boost::variant<uint32_t, std::string>> request_data = {50, "0x64", 200};
  EXPECT_OUTCOME_TRUE(
      r,
      api.getBlockHash(std::vector<boost::variant<uint32_t, std::string>>(
          {50, "0x64", 200})));
