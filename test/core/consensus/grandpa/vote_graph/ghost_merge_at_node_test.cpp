...
TEST_F(VoteGraphFixture, GhostMergeAtNodes) {
  ...
  "base_number": 0
    ...
    EXPECT_OK(graph->insert(vt, {2, "B"_H}, "w0_a"_ID));
  ...
  "base_number": 0
    ...
    EXPECT_OK(graph->insert(vt, {3, "C"_H}, "w5_a"_ID));
  ...
  "base_number": 0
    ...
    EXPECT_OK(graph->insert(vt, {5, "E1"_H}, "w5_b"_ID));
  ...
  "base_number": 0
    ...
    EXPECT_OK(graph->insert(vt, {6, "F2"_H}, "w5_c"_ID));
  ...
  auto check = [&](const std::optional<BlockInfo> &block) {
    auto ghostOpt =
        graph->findGhost(vt, block, [](auto &&x) { return x.sum(vt) >= 7; });
