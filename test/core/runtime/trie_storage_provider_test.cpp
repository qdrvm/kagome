...
class TrieStorageProviderTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    EXPECT_OK(storage_provider_->setToPersistentAt(
...
TEST_F(TrieStorageProviderTest, StartTransaction) {
  EXPECT_OK(storage_provider_->startTransaction());
}

TEST_F(TrieStorageProviderTest, FinishTransactionWithoutStart) {
  EXPECT_EC(storage_provider_->rollbackTransaction(),
            RuntimeExecutionError::NO_TRANSACTIONS_WERE_STARTED);

  EXPECT_EC(storage_provider_->commitTransaction(),
            RuntimeExecutionError::NO_TRANSACTIONS_WERE_STARTED);
}
...
TEST_F(TrieStorageProviderTest, NestedTransactions) {
  ...
  EXPECT_OK(batch0->put("A"_buf, "-"_buf));
  EXPECT_OK(batch0->put("B"_buf, "-"_buf));
  EXPECT_OK(batch0->put("C"_buf, "-"_buf));
  EXPECT_OK(batch0->put("D"_buf, "-"_buf));
  EXPECT_OK(batch0->put("E"_buf, "-"_buf));
  ...
  {  // Transaction 1 - will be committed
    ...
    EXPECT_OK(batch1->put("A"_buf, "1"_buf));
...

    {
      ...
      EXPECT_OK(batch2->put("B"_buf, "2"_buf));
...

      {
        ...
        EXPECT_OK(batch3->put("C"_buf, "3"_buf));
        ...
        /// @when 7. commit tx3
        EXPECT_OK(storage_provider_->commitTransaction());
      ...
      EXPECT_OK(batch2->put("D"_buf, "2"_buf));
      ...
      /// @when 9. rollback tx2
      EXPECT_OK(storage_provider_->rollbackTransaction());
    ...
    EXPECT_OK(batch1->put("E"_buf, "1"_buf));
...
TEST_F(TrieStorageProviderTest, ChildTreeTransactions) {
  auto base_batch_1 =
      EXPECT_OK(storage_provider_->getMutableChildBatchAt("child_root_1"_buf));
  EXPECT_OK(base_batch_1.get().put("A"_buf, "1"_buf));
  EXPECT_OK(base_batch_1.get().put("B"_buf, "2"_buf));
  EXPECT_OK(base_batch_1.get().put("C"_buf, "3"_buf));
  EXPECT_OK(base_batch_1.get().put("D"_buf, "-"_buf));
  EXPECT_OK(base_batch_1.get().put("E"_buf, "-"_buf));
  checkBatchValues(base_batch_1, "123--");

  auto base_batch_2 =
      EXPECT_OK(storage_provider_->getMutableChildBatchAt("child_root_2"_buf));
  EXPECT_OK(base_batch_2.get().put("A"_buf, "4"_buf));
  EXPECT_OK(base_batch_2.get().put("B"_buf, "5"_buf));
  EXPECT_OK(base_batch_2.get().put("C"_buf, "6"_buf));
  EXPECT_OK(base_batch_2.get().put("D"_buf, "-"_buf));
  EXPECT_OK(base_batch_2.get().put("E"_buf, "-"_buf));
  ...
  EXPECT_OK(tr1_batch_1.get().put("A"_buf, "a"_buf));
  EXPECT_OK(tr1_batch_2.get().put("A"_buf, "d"_buf));
  ...
  EXPECT_OK(tr2_batch_1.get().put("A"_buf, "0"_buf));
  EXPECT_OK(tr2_batch_2.get().put("A"_buf, "1"_buf));
  ...
  EXPECT_OK(tr3_batch_1.get().put("A"_buf, "a"_buf));
  EXPECT_OK(tr3_batch_2.get().put("A"_buf, "d"_buf));
