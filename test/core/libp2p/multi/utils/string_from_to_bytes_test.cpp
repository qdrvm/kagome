...
using libp2p::multi::converters::stringToBytes;
using libp2p::multi::converters::bytesToString;
using kagome::common::Buffer;
using kagome::common::unhex;

#define VALID_STR_TO_BYTES(addr, bytes) { \
auto res = stringToBytes(addr); \
...
auto res = bytesToString(Buffer{unhex(bytes).getValueRef()}); \
ASSERT_TRUE(res) << res.error().message(); \
