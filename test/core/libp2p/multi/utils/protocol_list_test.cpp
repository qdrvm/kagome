 ...
 * @then if a protocol with such name exists, a pointer to the data about it is returned
 */
 ...
 * @then if a protocol with such code exists, a pointer to the data about it is returned
 */
TEST(ProtocolList, getByCode) {
  static_assert(ProtocolList::get(Protocol::Code::ip6)->name == "ip6");
  static_assert(ProtocolList::get(Protocol::Code::dccp)->code == Protocol::Code::dccp);
 ...
 * @then the collection contatining the data about all known protocols is returned
 */
TEST(ProtocolList, getProtocols) {
  ...
  auto it = std::find_if(protocols.begin(), protocols.end(), [](auto& p){ return p.name == "ip4";});
