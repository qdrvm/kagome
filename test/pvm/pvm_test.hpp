...
class RiscVVM {
 public:
  ...
  void registerExternalFunction(
      uint64_t id, std::function<int64_t(int64_t *, uint64_t)> func) {
 ...
 private:
  ...
  std::unordered_map<uint64_t, std::function<int64_t(int64_t *, uint64_t)>>
      externalFunctions;
  ...
  void handleException(const std::string &cause, uint64_t faultAddr = 0) {
    std::lock_guard<std::mutex> lock(printMutex);
    std::cerr << "Exception: " << cause << " at address 0x" << std::hex
              << faultAddr << std::dec << std::endl;
  ...
  void callExternalFunction(uint64_t id) {
    ...
    uint64_t memArg =
        registers[18];  // a8 содержит адрес в памяти для дополнительных данных
  ...
  void executeInstruction(uint64_t instruction) {
    ...
    int64_t imm_b = ((instruction >> 31) << 12) | ((instruction >> 7) & 0x1E)
                  | ((instruction >> 25) << 5) | ((instruction >> 8) << 11);
    int64_t imm_u = instruction & 0xFFFFF000;
    int64_t imm_j = ((instruction >> 31) << 20) | ((instruction >> 21) << 1)
                  | ((instruction >> 20) << 11) | ((instruction >> 12) << 12);
    ...
    if (imm_b & 0x1000) {
      imm_b |= 0xFFFFFFFFFFFFE000;
    }
    if (imm_j & 0x100000) {
      imm_j |= 0xFFFFFFFFFFE00000;
    }
      ...
      case 0x33:
        switch (funct3) {
          ...
          case 0x2:
            ...
            break;  // SLT
          case 0x3:
            registers[rd] = (static_cast<uint64_t>(registers[rs1])
          ...
          case 0x5:  // SRL или SRA
            if (funct7 == 0x00) {
              registers[rd] = static_cast<uint64_t>(registers[rs1])
                           >> (registers[rs2] & 0x3F);  // SRL
            } else if (funct7 == 0x20) {
              registers[rd] = registers[rs1] >> (registers[rs2] & 0x3F);  // SRA
            }
            break;
          case 0x6:
            registers[rd] = registers[rs1] | registers[rs2];
      ...
      case 0x13:
        switch (funct3) {
          case 0x0:
            ...
            break;  // ADDI
          case 0x1:
            registers[rd] = registers[rs1] << (imm_i & 0x3F);
            break;  // SLLI
          case 0x2:
            registers[rd] = (registers[rs1] < imm_i) ? 1 : 0;
            break;  // SLTI
          case 0x3:
            registers[rd] = (static_cast<uint64_t>(registers[rs1])
                             < static_cast<uint64_t>(imm_i))
                              ? 1
                              : 0;
            break;  // SLTIU
          case 0x4:
          ...
          case 0x5:  // SRLI или SRAI
            if (funct7 == 0x00) {
              ...
              registers[rd] = registers[rs1] >> (imm_i & 0x3F);  // SRAI
            }
            break;
          case 0x6:
            registers[rd] = registers[rs1] | imm_i;
        ...
        switch (funct3) {
          case 0x0:  // SB
            if (!isAddressAligned(registers[rs1] + imm_s, 1)) {
              handleException("Store address misaligned",
                              registers[rs1] + imm_s);
              break;
            }
      ...
      case 0x63:
        switch (funct3) {
          ...
          case 0x5:
            ...
            break;  // BGE
          case 0x6:
            if (static_cast<uint64_t>(registers[rs1])
                < static_cast<uint64_t>(registers[rs2])) {
              pc += imm_b;
            } else {
              pc += 4;
      ...
      case 0x73:
        if (funct3 == 0x0) {
          callExternalFunction(
              registers[17]);  // a7 содержит идентификатор функции
        }
        pc += 4;
        break;
        ...
        if (funct7 == 0x01) {
          switch (funct3) {
            case 0x0:
              ...
              break;  // MUL
            case 0x1:
              registers[rd] = (static_cast<int128_t>(registers[rs1])
                               * static_cast<int128_t>(registers[rs2]))
                           >> 64;
              break;  // MULH
            case 0x2:
              registers[rd] = (static_cast<uint128_t>(registers[rs1])
                               * static_cast<uint128_t>(registers[rs2]))
                           >> 64;
              break;  // MULHU
            case 0x3:
              registers[rd] = (static_cast<int128_t>(registers[rs1])
...
int64_t readStructFunction(int64_t *args, uint64_t memArg) {
  RiscVVM *vm =
      ...
      reinterpret_cast<RiscVVM *>(
          args[0]);  // Передаем указатель на виртуальную машину
  ExampleStruct s =
      vm->readStruct<ExampleStruct>(memArg);  // Чтение структуры из памяти
  std::cout << "Struct fields: " << s.field1 << ", " << s.field2 << ", "
            << s.field3 << std::endl;
...
int64_t writeStructFunction(int64_t *args, uint64_t memArg) {
  RiscVVM *vm = reinterpret_cast<RiscVVM *>(
      args[0]);  // Передаем указатель на виртуальную машину
  ExampleStruct s;
